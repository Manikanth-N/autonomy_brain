import time
from models.states import TopLevelState, SubState
from models.transition_request import TransitionRequest

from interfaces.vehicle.mavlink_interface import VehicleInterface as vehicle

class MainLoop:
    def __init__(self, 
                 event_queue, 
                 safety, 
                 system_manager, 
                 controller_manager, 
                 state_machine, 
                 mission_manager, 
                 timeout_supervisor, 
                 vehicle
                 ):
        self.event_queue = event_queue
        self.safety = safety
        self.system_manager = system_manager
        self.controller_manager = controller_manager
        self.state_machine = state_machine
        self.mission_manager = mission_manager
        self.timeout_supervisor = timeout_supervisor
        self.vehicle = vehicle


        self.running = True
        self._boot_transition_done = False
        self._illegal_test_done = False
        self.link_lost_triggered = False

    def run(self):
        loop_rate = 20
        dt = 1.0 / loop_rate

        while self.running:
            start = time.time()

            # Boot transition
            if not self._boot_transition_done:
                tr = TransitionRequest(
                    source="System",
                    target_top=TopLevelState.GROUND,
                    target_sub=SubState.STANDBY,
                    reason="BootComplete"
                )
                self.system_manager.request_transition(tr)
                self._boot_transition_done = True
            
            if self.vehicle.connected:
                if time.time() - self.vehicle.last_heartbeat > 2:
                    if not self.link_lost_triggered:
                        print("⚠ Heartbeat timeout detected")
                        self.event_queue.push({"type": "LINK_LOST"})
                        self.link_lost_triggered = True
                else:
                    self.link_lost_triggered = False
                    
            # -----------------------------------
            # Process events
            # -----------------------------------
            events = self.event_queue.pop_all()
            for event in events:
                self.mission_manager.handle_event(event)
                self.safety.handle_event(event)

            # -----------------------------------
            # Safety evaluation
            # -----------------------------------
            self.safety.evaluate()
            self.system_manager.evaluate()

            # -----------------------------------
            # Mission logic
            # -----------------------------------
            self.mission_manager.update()
            self.system_manager.evaluate()

            # -----------------------------------
            # State timeout supervision
            # -----------------------------------
            self.timeout_supervisor.update()
            self.system_manager.evaluate()

            # -----------------------------------
            # Controllers
            # -----------------------------------
            state = self.state_machine.get_state()
            print("Current State:", state)
            self.controller_manager.update(state)

            # -----------------------------------
            # Loop timing
            # -----------------------------------
            elapsed = time.time() - start
            sleep_time = dt - elapsed
            if sleep_time > 0:
                time.sleep(sleep_time)